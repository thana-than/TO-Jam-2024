...
public static class Extensible
{
    ...
    public static T[] GetAllFieldsOfType<T, C>(C _class)
    {
        return _class.GetType().GetFields()
            .Where(x => x.FieldType == (typeof(T)))
            .Select(x => (T)x.GetValue(_class)).ToArray();
    }

    public static T[] GetAllPropertiesOfType<T, C>(C _class)
    {
        return _class.GetType().GetProperties()
    ...
    // public static float StartTime(this AnimationCurve curve) => curve.keys[0].time;
    //public static float EndTime(this AnimationCurve curve) => curve.keys[curve.keys.Length - 1].time;
    ...
    public static IEnumerator Animate(this AnimationCurve curve, System.Action<float> stepAction, bool useScaledDeltaTime = true, Func<float> deltaTimeModifier = null)
    {
        bool hasTimeMod = deltaTimeModifier != null;

        float GetDeltaTime()
        {
            return useScaledDeltaTime ? Time.deltaTime : Time.unscaledDeltaTime;
        }

        float endTime = curve.LastTime();
        for (float t = 0; t < endTime; t += hasTimeMod ? deltaTimeModifier.Invoke() * GetDeltaTime() : GetDeltaTime())
        {
    ...
    /// <param name="deltaTimeModifier">Callback with a float return type that can be used to modify the speed that the coroutine progresses.</param>
    /// <returns>IEnumerator: Should be used within the Monobehaviour StartCoroutine(IEnumerator coroutine) method.</returns>
    public static IEnumerator AnimateReverse(this AnimationCurve curve, System.Action<float> stepAction, bool useScaledDeltaTime = true, Func<float> deltaTimeModifier = null)
    {
        bool hasTimeMod = deltaTimeModifier != null;
        ...
        for (float t = curve.LastTime(); t > 0; t -= hasTimeMod ? deltaTimeModifier.Invoke() * GetDeltaTime() : GetDeltaTime())
        {
            stepAction.Invoke(curve.Evaluate(t));
    ...
    public static string[] GetExposedParameters(this AudioMixer mixer)
    {
        //Exposed Params
        Array props = (Array)mixer.GetType().GetProperty("exposedParameters").GetValue(mixer, null);
        int len = props.Length;

        string[] parameters = new string[len];
        for (int i = 0; i < len; i++)
        {
            var o = props.GetValue(i);
            parameters[i] = (string)o.GetType().GetField("name").GetValue(o);
        }
